...
## 화면 (브라우저 캔버스 + 파이썬 엔진 — A안)

엔진은 화면 없이 완결돼 있고, 화면은 브라우저다. 파이썬이 로컬에서 작은 서버로 돌고
브라우저가 SVG 캔버스에 그린다. 표준 라이브러리만 쓰므로 따로 설치할 게 없다.

```
python -m patterncad.server          # http://127.0.0.1:8765 를 브라우저로 연다
```

- 위: **새 패턴**(마법사), 스타일·원형 고르기, 프로젝트 열기·저장, 단위(inch 분수 / cm / mm), 수정 초기화, 맞춤, SVG 저장
- 왼쪽: 조각 나무(누르면 그 조각으로 확대), 층(역할별), 점 이름·보조점, **원본 도면 겹쳐 보기**(조각별), 원형 이력
- 가운데 캔버스: 끌어 이동, 휠 확대(`F` 맞춤, `Esc` 선택 해제). 점·선을 누르면 오른쪽에 **그 점의 규칙**(식 그대로)과 지나는 선.
  - **점 끌기 → 수정값**: 치수를 바꿔도 손으로 옮긴 점은 그 자리, 그 점을 참조하는 뒷 점은 따라온다 (`point_overrides`). `Delete` 로 되돌림
  - **곡선 핸들 끌기**: 곡선을 고르면 파란 핸들이 나오고 끌 수 있다. 현(구간 양 끝) 기준 비율·각으로 저장해
    치수가 바뀌어도 모양이 따라온다 (`line_overrides`). 이어진 구간의 반대쪽 핸들은 같은 방향을 보게 맞춘다
  - **자**: 점을 고른 뒤 다른 점을 Shift+클릭하면 거리(가로·세로 포함)
- 오른쪽: 원형별 치수표. 노란 칸이 고친 값. `value`·`choice` 는 고칠 수 있고, `formula`·`table` 은 식과 결과만,
  스타일이 다른 원형에서 받아 오는 값(예: 소매 앞AH ← len(body.앞암홀))은 잠겨 있다
- **원본 도면 겹쳐 보기**: 조각을 켜면 `tools/align_block.py` 로 원형을 포트폴리오 도면에 맞춘 변환을 구해
  (처음 한 번 20 초쯤, `verify/fits.json` 에 남는다) 추출한 도면의 패턴 층을 빨갛게 겹쳐 그린다. 치수 글자까지 같이 보인다
...
- **조각·시접 보기**(위쪽 토글): 조각마다 외곽선을 이어 닫고 변마다 시접을 붙인 **재단선**(파랑), 노치(다트 다리·노치 표시·끊긴 자리),
  ...
  옆솔기 가슴다트처럼 외곽선이 벌어진 자리는 곧게 이어 붙이고 양 끝에 노치를 두며, 재단선에는 **다트 캡**(접는 쪽 이웃 재단선을
  다트 중심선까지 연장한 꼭짓점 — 기본 아래로 접음, 조각 설정에서 위로/끄기)을 붙인다. 바지 주름선처럼 조각 가운데를 지나는 접는 선은 안쪽 선
- **그레이딩**(왼쪽 패널): 사이즈 체계(`data/sizes.yaml` — KS 여성복 44~88, S/M/L, 초안)와 기준 호칭을 고르고 사이즈를 켜면
  **치수 재대입**으로 다시 그린 원형이 색색으로 겹쳐 보인다. 기준 사이즈에서 손본 치수·점·핸들은 그대로 두고 표의 **차이만** 더한다.
  켠 사이즈는 DXF 에 함께 나간다 (조각 이름에 호칭, 사이즈마다 한 줄). 점을 고르면 **사이즈별 편차**(가로·세로)를 적을 수 있어
  치수 재대입 위에 점별 편차 규칙을 얹는다 (프로젝트에 저장). 마카에는 켠 사이즈 조각이 매수대로 섞여 놓인다
...
- **마카**(위쪽 토글): 원단 폭을 정하고 조각을 매수대로(둘째 장은 뒤집어) 놓는다. 끌어 옮기기(1/8" 눈금), `R` 90° 돌리기, `M` 뒤집기,
  겹치면 빨강·원단 밖이면 점선. **자동 네스팅**은 1/4" 격자 비트맵에 큰 조각부터 왼쪽 아래 빈자리를 찾아 놓는 방식으로
  결(0°·180°)을 지키고, "90° 허용"을 켜면 네 방향을 다 본다 (블라우스 12장: 선반 2.00 yd·37% → 네스팅 1.21 yd·62%). **요척**(yd·m)과 효율을 바로 보여 주고
  마카 DXF(놓인 자리대로 한 층)·SVG 로 내보낸다. 놓은 자리는 프로젝트 파일에 저장
- 선의 `cut_piece`(재단 조각 이름, 여럿 가능)와 `cut: false`(그림에만): 고어드 스커트는 그림 앞판·뒤판, 재단 고어 셋씩;
  두 장 소매는 겹쳐 그린 선 대신 앞·뒤 띠를 밑선에서 이어 붙인 **작은소매(펼침)** 조각으로 재단
...
서버는 상태가 없다 — 수정값은 브라우저가 들고 있다가 계산할 때마다 보내고, 저장은 프로젝트 파일로 한다.
